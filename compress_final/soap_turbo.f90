...
!**************************************************************************
  subroutine get_soap_gpu(n_sites, n_neigh, n_species, species, species_multiplicity, n_atom_pairs, mask, rjs, &
...
! Internal variables
  ...
  integer(c_size_t) :: st_size_n_neigh, st_size_k2_start, st_size_k2_i_site, st_size_i_k2_start
  integer(c_size_t) :: st_size_k3_index
  type(c_ptr) :: gdorpro_azi_d, gdorpro_rad_d, gdorpro_pol_d
  type(c_ptr) :: transp_azi_d, transp_rad_d, transp_pol_d
  real(c_double), allocatable,target :: g_this_soap(:,:)
  real(c_double), allocatable,target :: g_this_soap_rad_der(:,:), g_this_soap_azi_der(:,:), g_this_soap_pol_der(:,:)
  ...
  integer(c_size_t) :: st_size_g_this_soap
  type(c_ptr) :: g_this_soap_d
...
! Uncompressed SOAP dimensions
  ...
  ! write(*,*) 
  ! write(*,*) 
  ! write(*,*) n_soap_uncompressed
  ! write(*,*)  maxval(compress_P_i)
  ! write(*,*) 
  ! write(*,*) 
  ! stop
...
!   New way to compress with a linear transformation
  ...
  if( do_derivatives )then
    ...
    allocate( gdorpro_azi(1:n_soap, 1:n_atom_pairs) )
    allocate( gdorpro_pol(1:n_soap, 1:n_atom_pairs) )
    allocate( gdorpro_rad(1:n_soap, 1:n_atom_pairs) )
    allocate( t_azi(1:n_soap, 1:n_atom_pairs) )
    allocate( t_pol(1:n_soap, 1:n_atom_pairs) )
    allocate( t_rad(1:n_soap, 1:n_atom_pairs) )
...
! Create the multiplicity array
  ...
  allocate( g_this_soap(1:n_soap_uncompressed,1:n_sites) )
  g_this_soap=0.0
  do i = 1, n_sites
    this_soap = 0.d0
    counter = 0
...
!            end if
            ...
            g_this_soap(counter,i) = g_this_soap(counter,i) + multiplicity * real(cnk(k, n, i) * conjg(cnk(k, np, i)))
          end do
        end do
      end do
    end do
...
!   for large dimensions and BLAS routines if P is dense)
    if( compress_soap )then
      do k = 1, compress_P_nonzero
        ...
        soap(compress_P_i(k), i) = soap(compress_P_i(k), i) + compress_P_el(k) * g_this_soap(compress_P_j(k),i)
      end do
    else
      ! soap(:,i) = this_soap(:)
      soap(:,i) = g_this_soap(:,i)
    end if
  end do
  ...
  st_size_g_this_soap=sizeof(g_this_soap) 
  call gpu_malloc_all(g_this_soap_d,st_size_g_this_soap,gpu_stream)
  call cpy_htod(c_loc(g_this_soap),g_this_soap_d,st_size_g_this_soap,gpu_stream)
  ...
  call gpu_memset_async(sqrt_dot_p_d,valuetoset,st_sqrt_dot_p,gpu_stream)
  call gpu_memset_async(soap_d,valuetoset,st_soap_d,gpu_stream)
  ...
  call gpu_get_sqrt_dot_p(sqrt_dot_p_d, soap_d, g_this_soap_d,  multiplicity_array_d, &
...
!****************************
    ...
    st_size_g_this_soap_pol_der=sizeof(g_this_soap_pol_der)

    call gpu_malloc_all(g_this_soap_rad_der_d,st_size_g_this_soap_rad_der,gpu_stream)
    call gpu_malloc_all(g_this_soap_azi_der_d,st_size_g_this_soap_azi_der,gpu_stream)
    call gpu_malloc_all(g_this_soap_pol_der_d,st_size_g_this_soap_pol_der,gpu_stream)

    call cpy_htod(c_loc(g_this_soap_rad_der),g_this_soap_rad_der_d,st_size_g_this_soap_rad_der,gpu_stream)
    ...
    st_size_k3_index=sizeof(k3_index)
    call gpu_malloc_all(k3_index_d,st_size_k3_index,gpu_stream)
    call cpy_htod(c_loc(k3_index),k3_index_d, st_size_k3_index, gpu_stream)
...
!    call cpy_htod(c_loc(soap_cart_der),soap_cart_der_d,st_size_soap_cart_der,gpu_stream)
  ...
  ! enddo
    ...
    call gpu_free_async(transp_azi_d,gpu_stream)
    call gpu_free_async(transp_pol_d,gpu_stream)
    call gpu_free_async(transp_rad_d,gpu_stream)
...
!   open(unit=10, file="soap_desc.dat", status="unknown", access="append")
   ...
   close(10)
   open(unit=10, file="soap_rad_der.dat", status="unknown", position="append")
   write(10, *) soap_rad_der(1:n_soap, 2)
   close(10)
   open(unit=10, file="soap_azi_der.dat", status="unknown", position="append")
   write(10, *) soap_azi_der(1:n_soap, 2)
   close(10)
   open(unit=10, file="soap_pol_der.dat", status="unknown", position="append")
